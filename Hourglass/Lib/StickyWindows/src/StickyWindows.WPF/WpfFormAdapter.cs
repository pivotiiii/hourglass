    ...
    public class WpfFormAdapter : BaseFormAdapter
    {
        ...
        protected override Rectangle InternalBounds
        {
            get
            {
                ...
                var widthHeightPointConverted =
                    FromRelativeToDevice(_window.ActualWidth, _window.ActualHeight, _window);
                ...
                var pEnd = PointToScreen(origin + new Size(Convert.ToInt32(widthHeightPointConverted.X),
        ...
        {
            get => new(
                Convert.ToInt32(_window.MaxWidth > int.MaxValue ? int.MaxValue : _window.MaxWidth),
                Convert.ToInt32(_window.MaxHeight > int.MaxValue ? int.MaxValue : _window.MaxHeight));
            set
            {
        ...
        {
            get => new(
                Convert.ToInt32(_window.MinWidth < int.MinValue ? int.MinValue : _window.MinWidth),
                Convert.ToInt32(_window.MinHeight < int.MinValue ? int.MinValue : _window.MinHeight));
            set
            {
        ...
        {
            var p = new WindowsPoint();
            var resultWpf = ToWinPoint(_window.PointToScreen(p));
            var resultScaled = resultWpf + new Size(point);
            return resultScaled;
        }
        ...
        private static WindowsPoint FromDeviceToRelative(double x, double y, Visual workingVisual)
        {
        ...
        private static WindowsPoint FromRelativeToDevice(double x, double y, Visual workingVisual)
        {
            var widthHeightPoint = new Point(Convert.ToInt32(x), Convert.ToInt32(y));
            var source = PresentationSource.FromVisual(workingVisual);
        ...
        private static WindowsPoint ToWpfPoint(Point point)
        {
            return new WindowsPoint(point.X, point.Y);
        }

        private static Point ToWinPoint(WindowsPoint point)
        {
