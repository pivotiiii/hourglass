...
namespace StickyWindows
{
    ...
    public class StickyWindow : NativeWindow
    {
        ...
        /// </summary>
        private static readonly List<BaseFormAdapter> _globalStickyWindows = new();

        private const int StickGap = 20; // distance to stick
        ...
        private bool DefaultMsgProcessor(ref Message m)
        {
            switch (m.Msg)
            {
                case Win32.WM.WM_NCLBUTTONDOWN:
                {
                    ...
                    if (OnNonClientLeftButtonDown((int) m.WParam, Win32.LParam.GetPoint(m.LParam)))
                    {
        ...
        private bool ResizeMsgProcessor(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                ...
                {
                    Resize(Win32.LParam.GetPoint(m.LParam));
                    break;
                }
        ...
        private void Resize(Point p)
        {
            ...
            if ((_resizeDirection & ResizeDir.Top) == ResizeDir.Top)
            {
                ...
                iNewHeight = Math.Max(iNewHeight, 16); // TODO: GetSystemMetrics icon?
        ...
        private bool MoveMsgProcessor(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                ...
                case Win32.WM.WM_MOUSEMOVE:
                {
                    Move(Win32.LParam.GetPoint(m.LParam));
                    break;
                }
                case Win32.WM.WM_KEYDOWN:
                {
        ...
        private void Move_Stick(Rectangle toRect, bool bInsideStick)
        {
            ...
            if (_formRect.Bottom >= toRect.Top - StickGap && _formRect.Top <= toRect.Bottom + StickGap)
            {
                ...
                if (Math.Abs(_formRect.Left + _formRect.Width - toRect.Left - toRect.Width) <=
                    Math.Abs(_formOffsetPoint.X))
                {
                    // snap right 2 right
            ...
            if (_formRect.Right >= toRect.Left - StickGap && _formRect.Left <= toRect.Right + StickGap)
            {
                ...
                if (Math.Abs(_formRect.Top + _formRect.Height - toRect.Top - toRect.Height) <=
                    Math.Abs(_formOffsetPoint.Y))
                {
                    // bottom 2 bottom
